function [ Y ] = ADSR_env( Z, beat_length, num_beats, fs )
    ...
    ADSR = [0.05, 0.2, 0.5, 0.25]; % envelope should add up to 1
    ...
    for i = 1:length(strums)
        ...
        F(d+1:s) = linspace(peak, last_sustain, s-d);
        F(s+1:r) = linspace(last_sustain, last_sustain, r-s);
        F(r+1:k) = linspace(last_sustain, 0, k-r);
        E(ind+1:ind+k) = F;
        ind = ind + k;
        last_sustain = last_sustain - ((peak - mid) / length(strums)); % decrease the sustain level after each strum
    end
